...
export async function createGap(input: { workspaceId: string; projectId: string; claimId?: string; proofAttemptId?: string; routeId?: string; title: string; descriptionMarkdown: string; severity?: string; status?: string; suggestedResolution?: string; resolutionKind?: string; resolutionRole?: string; frontierEligible?: boolean; targetObjectType?: string; targetObjectId?: string; externalReviewId?: string; auditFindingId?: string }) {
  const gap = await prisma.gap.create({
    data: {
      ...
      frontierEligible: input.frontierEligible ?? true,
      targetObjectType: input.targetObjectType,
      targetObjectId: input.targetObjectId,
