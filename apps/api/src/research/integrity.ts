...
const frontierWritesByRole: Partial<Record<AgentRole, string[]>> = {
  ProjectCoordinator: ["Project", "ProjectGoal", "Workstream", "AgentMessage", "RunOutcome"],
  GapAnalyst: ["Gap", "Workstream", "WorkstreamReport", "AgentMessage", "RunOutcome"],
  ProofAttemptAgent: ["ProofAttempt", "Gap", "Assumption", "WorkstreamReport", "AgentMessage", "RunOutcome"],
  CodingAgent: ["Experiment", "Artifact", "Gap", "WorkstreamReport", "AgentMessage", "RunOutcome"],
  PaperWriter: ["ManuscriptDocument", "ManuscriptSection", "ManuscriptVersion", "ProofObligation", "PaperBuild", "WorkstreamReport", "AgentMessage", "RunOutcome"]
}
...
export async function createReviewAssignment(input: { workspaceId: string; projectId: string; workstreamId: string; reviewerRunId: string; reviewType: string; targetObjectType: string; targetObjectId: string; targetHash?: string; manuscriptVersionId?: string; permittedArtifactIds?: string[]; briefing: unknown; leaseExpiresAt: Date }) {
  ...
  await prisma.reviewAssignment.updateMany({ where: { workspaceId: input.workspaceId, workstreamId: input.workstreamId, status: "claimed", reviewerRun: { status: { notIn: ["started", "running", "submitted"] } } }, data: { status: "cancelled" } })
  const activeAssignment = await prisma.reviewAssignment.findFirst({ where: { workspaceId: input.workspaceId, workstreamId: input.workstreamId, status: "claimed", leaseExpiresAt: { gt: new Date() }, reviewerRun: { status: { in: ["started", "running", "submitted"] } } } })
...
export async function runProjectGraphAudit(input: { workspaceId: string; projectId: string; mode: string; auditorRunId?: string }) {
  ...
  for (const version of versions.filter((v) => v.isCanonical)) {
    ...
    const unhealthyPhysical = version.physicalArtifacts.filter((link) => link.artifact.storageStatus !== "available" || !link.artifact.storageKey || !link.artifact.sha256 || link.artifact.byteSize === null)
    if (unhealthyPhysical.length) findings.push({ severity: "major", category: "artifact_integrity", title: "Attached manuscript artifacts are unavailable or incomplete", descriptionMarkdown: `Version ${version.id} links ${unhealthyPhysical.length} physical artifact records whose managed bytes or integrity metadata are unavailable.`, targetObjectType: "ManuscriptVersion", targetObjectId: version.id, evidence: unhealthyPhysical.map((link) => link.artifactId), proposedRepair: "Recover or regenerate the affected exact bytes and attach a valid immutable artifact. Missing release outputs that have never been claimed remain ordinary readiness work, not an audit defect." })
  }
