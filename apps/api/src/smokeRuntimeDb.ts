...
const routedGoalProposal = await runtime.proposeProjectGoal({ workspaceId: workspace.id, projectId: routedProject.id, title: "Repair the manuscript frontier", statement: "Route the executable manuscript leaf without reviving parent or spinout work.", successCriteria: ["The PaperWriter leaf is the sole runnable frontier."] })
const routedGoal = await runtime.approveProjectGoal({ workspaceId: workspace.id, goalId: routedGoalProposal.id, userId: user.id })
...
const staleParentWorkstream = await runtime.createWorkstream({ workspaceId: workspace.id, projectId: routedProject.id, goalId: routedGoal.id, title: "Stale parent analysis", kind: "gap_analysis", coordinatorRole: "GapAnalyst", targetObjectType: "Gap", targetObjectId: parentGap.id, instructions: "Repeat the parent analysis." })
const staleSpinoutWorkstream = await runtime.createWorkstream({ workspaceId: workspace.id, projectId: routedProject.id, goalId: routedGoal.id, title: "Stale spinout analysis", kind: "gap_analysis", coordinatorRole: "GapAnalyst", targetObjectType: "Gap", targetObjectId: spinoutGap.id, instructions: "Incorrectly dispatch spinout work." })
...
assert.ok(controlRoom.workstreams_by_status.completed?.some((item) => item.id === workstream.id))
  ...
  await assert.rejects(() => runtime.claimNextReview({ userId: user.id, workspaceRef: workspace.id, project: project.id, sessionId: `explicit-integrated-review-${suffix}`, model: "smoke" }), /REVIEW_BUILD_MISSING/)
  assert.equal(await prisma.reviewAssignment.count({ where: { workstreamId: explicitIntegratedReview.id } }), 0, "missing exact build must fail before an assignment is created")
  assert.equal((await prisma.workstream.findUniqueOrThrow({ where: { id: explicitIntegratedReview.id } })).status, "needs_review", "preflight failure must not consume the workstream")
  ...
  await prisma.workstreamReport.update({ where: { id: explicitIntegratedReport.id }, data: { status: "reviewed_needs_revision" } })
...
try {
  ...
  await prisma.paperBuild.create({ data: { workspaceId: workspace.id, projectId: project.id, manuscriptVersionId: manuscriptVersion.id, status: "succeeded", builderVersion: `smoke-exact-${suffix}`, sourceHash: originalHash, sourceArtifactId: durableArtifact.id, pdfArtifactId: compiledPdf.id, buildManifest: { fixture: true }, completedAt: new Date() } })
  await runtime.attachArtifactToManuscriptVersion({ workspaceId: workspace.id, artifactId: durableArtifact.id, manuscriptVersionId: successorVersion.id, role: "source_bundle" })
  await runtime.attachArtifactToManuscriptVersion({ workspaceId: workspace.id, artifactId: compiledPdf.id, manuscriptVersionId: successorVersion.id, role: "compiled_pdf" })
  const successorBuild = await prisma.paperBuild.create({ data: { workspaceId: workspace.id, projectId: project.id, manuscriptVersionId: successorVersion.id, status: "succeeded", builderVersion: `smoke-successor-${suffix}`, sourceHash: successorVersion.contentHash, sourceArtifactId: durableArtifact.id, pdfArtifactId: compiledPdf.id, buildManifest: { fixture: true, successor: true }, completedAt: new Date() } })
  const canonicalManuscript = await runtime.getManuscriptVersion(workspace.id, manuscriptVersion.id)
  assert.equal(canonicalManuscript.isCanonical, false, "explicit successor activation must supersede the earlier working manuscript")
...
const legacyMathematicalReviewWorkstream = await runtime.createWorkstream({ workspaceId: workspace.id, projectId: project.id, goalId: goal.id, title: "Legacy mathematical-truth review alias", kind: "hostile_review", instructions: "Review the submitted theorem report.", coordinatorRole: "HostileReviewer", priority: 100, targetObjectType: "WorkstreamReport", targetObjectId: draftPhysicalSubmission.id, reviewPolicy: { min_approved_rounds: 1, review_type: "mathematical_truth", locked_assignment_required: true } })
...
const boundedSuccessorHandoff = await runtime.createWorkstream({ workspaceId: workspace.id, projectId: project.id, goalId: goal.id, title: "Bounded revision input and successor output", kind: "paper_synthesis", instructions: "Review the explicitly submitted successor, retaining the predecessor only as input provenance.", coordinatorRole: "PaperWriter", priority: 150, targetObjectType: "ManuscriptVersion", targetObjectId: manuscriptVersion.id, reviewPolicy: { min_approved_rounds: 1, review_type: "proof_integration", bounded_revision: true } })
const boundedSuccessorReport = await runtime.createOrUpdateWorkstreamReport({ workspaceId: workspace.id, workstreamId: boundedSuccessorHandoff.id, title: "Exact bounded successor", bodyMarkdown: "The predecessor was the repair input; this report submits one exact successor output.", linkedObjectRefs: [`ManuscriptVersion:${successorVersion.id}`, `PaperBuild:${successorBuild.id}`], artifactRefs: [durableArtifact.id, compiledPdf.id] })
await runtime.submitReportForReview({ workspaceId: workspace.id, reportId: boundedSuccessorReport.id })
const successorTargetClaim = await runtime.claimNextReview({ userId: user.id, workspaceRef: workspace.id, project: project.id, sessionId: `successor-target-${suffix}`, model: "smoke" })
assert.equal(successorTargetClaim.assignment.id, boundedSuccessorHandoff.id)
assert.equal(successorTargetClaim.review_assignment.assignment.targetObjectId, successorVersion.id, "submitted bounded-revision successor must override historical predecessor input")
assert.equal(successorTargetClaim.review_assignment.assignment.manuscriptVersionId, successorVersion.id)
assert.equal(successorTargetClaim.briefing.manuscript_inspection.build_id, successorBuild.id)
assert.equal(successorTargetClaim.briefing.review_target_resolution.predecessor_input_target_id, manuscriptVersion.id)
assert.equal(successorTargetClaim.briefing.review_target_resolution.reviewed_successor_id, successorVersion.id)
await prisma.reviewAssignment.update({ where: { id: successorTargetClaim.review_assignment.assignment.id }, data: { status: "cancelled" } })
await prisma.agentRun.update({ where: { id: successorTargetClaim.agent_run.id }, data: { status: "cancelled", finishedAt: new Date() } })
await prisma.workstream.update({ where: { id: boundedSuccessorHandoff.id }, data: { status: "abandoned", claimedSessionId: null, assignedToUserId: null, leaseExpiresAt: null } })
await prisma.workstreamReport.update({ where: { id: boundedSuccessorReport.id }, data: { status: "reviewed_needs_revision" } })
...
const integrationPayload = { workspaceId: workspace.id, workstreamId: lockedReviewWorkstream.id, verdict: "approved", reviewType: "proof_integration", targetVersion: manuscriptVersion.id, bodyMarkdown: integrationEvidence, issues: [], requiredChanges: [], checkedRefs: [sourceA.id, durableArtifact.id], obligationChecks: [{ proofObligationId: obligation.id, status: "preserved", evidenceMarkdown: "The exact source proof, hypotheses, excluded endpoint regime, and manuscript Lemma 4.1 were compared line by line.", expositionStatus: "journal_verifiable", completenessEvidence: { intermediate_arguments: "All intermediate inequalities are displayed.", uniformity_or_domination: "The uniform majorant and its domain are explicit.", endpoints_and_exceptional_cases: "The moving-start endpoint is checked.", normalizers_and_denominators: "Every denominator is nonzero on the stated domain.", notation_and_quantifiers: "Quantifiers and moving-start notation are defined before use.", external_theorem_applicability: "No external theorem is used." } }], createdByAgentRunId: reviewerRun.agentRun.id, reviewAssignmentId: lockedReview.assignment.id, submissionToken: lockedReview.submission_token, evidenceSections: [{ review_type: "proof_integration", conclusion: "approved", concrete_evidence: integrationEvidence, checked_refs: [sourceA.id, durableArtifact.id] }] }
const reviewsBeforeDryRun = await prisma.reviewRound.count({ where: { reviewAssignmentId: lockedReview.assignment.id } })
const validation = await runtime.recordReviewRound({ ...integrationPayload, validateOnly: true })
assert.equal(validation.valid, true)
assert.equal(validation.normalized_evidence_sections[0].sectionType, "proof_integration")
assert.equal(await prisma.reviewRound.count({ where: { reviewAssignmentId: lockedReview.assignment.id } }), reviewsBeforeDryRun, "validation-only must not persist or consume")
assert.equal((await prisma.reviewAssignment.findUniqueOrThrow({ where: { id: lockedReview.assignment.id } })).status, "claimed")
const [integrationReview, concurrentReplay] = await Promise.all([runtime.recordReviewRound(integrationPayload), runtime.recordReviewRound(integrationPayload)])
assert.equal(integrationReview.evidenceStatus, "assigned_valid")
assert.equal(concurrentReplay.id, integrationReview.id, "concurrent identical deliveries must converge on one immutable ReviewRound")
const replayedIntegrationReview = await runtime.recordReviewRound(integrationPayload)
assert.equal(replayedIntegrationReview.id, integrationReview.id)
assert.equal(replayedIntegrationReview.delivery_outcome, "replayed")
assert.equal(await prisma.reviewRound.count({ where: { reviewAssignmentId: lockedReview.assignment.id } }), 1)
...
await assert.rejects(() => runtime.recordReviewRound({ ...integrationPayload, bodyMarkdown: `${integrationEvidence} Divergent replay.` }), /REVIEW_REPLAY_CONFLICT/i)
...
const replayedOutcome = await runtime.submitRunOutcome({ workspaceId: workspace.id, agentRunId: reviewerRun.agentRun.id, completedWork: ["Different retry bytes are ignored because the durable outcome already exists."], nextAction: { kind: "blocked_by_tooling" } })
assert.equal(replayedOutcome.outcome.id, reviewerOutcome.outcome.id)
assert.equal(replayedOutcome.delivery_outcome, "replayed")
