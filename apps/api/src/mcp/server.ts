...
export const mcpServerVersion = "1.6.0-submission-candidate"
...
export const toolDefinitions: ToolDef[] = [
  ...
  tool("set_manuscript_freeze", "Set a lexical/interface/mathematical freeze, or explicitly move an exact manuscript between draft, integrated, and submission_candidate with lifecycle_stage. Only submission_candidate activates final proof/exposition and release assessment; provide bounded load_bearing_obligation_ids at that transition.", "editor", objectSchema({ workspace_id: s, manuscript_version_id: s, level: s, lifecycle_stage: s, load_bearing_obligation_ids: strArray }, ["workspace_id", "manuscript_version_id"]), scopes.maffReview),
...
export async function callTool(toolName: string, args: any, ctx: ToolContext) {
  ...
  switch (toolName) {
    ...
    case "set_manuscript_freeze": {
      if (args.lifecycle_stage) return runtime.setManuscriptLifecycle({ workspaceId, manuscriptVersionId: args.manuscript_version_id, stage: args.lifecycle_stage, loadBearingObligationIds: args.load_bearing_obligation_ids })
      if (!args.level) throw new Error("set_manuscript_freeze requires level or lifecycle_stage.")
      return runtime.setManuscriptFreeze({ workspaceId, manuscriptVersionId: args.manuscript_version_id, level: args.level })
    }
