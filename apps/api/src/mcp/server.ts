...
export const mcpServerVersion = "0.7.0-release-gate-plan"
...
export const toolDefinitions: ToolDef[] = [
  ...
  tool("record_review_round", "Record a scoped review verdict against an exact manuscript id, content hash, or version number. Approved proof-integration reviews must cover every required obligation; checked_obligation_ids are normalized into preserved checks, while explicit obligation_checks may use preserved/passed, partial, omitted, or failed.", "editor", objectSchema({ workspace_id: s, workstream_id: s, report_id: s, target_object_type: s, target_object_id: s, reviewer_role: s, verdict: reviewVerdict, review_type: s, target_version: s, scope: anyObj, inspected_artifact_ids: strArray, checked_obligation_ids: strArray, parent_math_reopenable: { type: "boolean" }, prior_approvals_evidence_only: { type: "boolean" }, independence: s, obligation_checks: { type: "array", items: anyObj }, issues: strArray, required_changes: strArray, checked_refs: strArray, body_markdown: s, created_by_agent_run_id: s }, ["workspace_id", "workstream_id", "verdict", "body_markdown"]), scopes.maffReview),
