...
const reviewEvidenceSectionSchema = { type: "object", properties: { review_type: s, conclusion: s, concrete_evidence: s, checked_refs: strArray, external_sources: { type: "array", items: anyObj }, attack_categories: strArray }, required: ["review_type", "conclusion", "concrete_evidence"], additionalProperties: true } as const
...
const releaseContractOutputSchema: JsonSchema = objectSchema({
  ...
  state: s,
  alignment: { anyOf: [objectOutputSchema, { type: "null" }] },
  authoritative_ids: objectSchema({ current_working_manuscript_version_id: nullableString, active_release_candidate_version_id: nullableString, reviewed_successor_manuscript_version_id: nullableString, supporting_review_round_id: nullableString, supporting_paper_build_id: nullableString }, ["current_working_manuscript_version_id", "active_release_candidate_version_id"]),
  manuscript_authority: objectSchema({ current_working_manuscript_version_id: nullableString, is_current_working_version: { type: "boolean" }, canonical_semantics: s, canonical_activation_confers_approval: { type: "boolean" }, release_assessment_active: { type: "boolean" }, active_release_candidate_version_id: nullableString, approval_status: s }, ["current_working_manuscript_version_id", "is_current_working_version", "canonical_semantics", "canonical_activation_confers_approval", "release_assessment_active", "active_release_candidate_version_id", "approval_status"]),
  ...
  enforcement: s
}, ["schema_version", "authority", "policy_version", "state", "alignment", "authoritative_ids", "manuscript_authority", "invariant_truths", "blockers", "next_action", "permitted_mutation_tools", "prohibited_shortcuts", "enforcement"])
...
export const mcpServerVersion = "1.11.0-exact-review-transactions"
export const expectedMcpToolCount = 115
...
export const toolDefinitions: ToolDef[] = [
  ...
  tool("assess_project_release_alignment", "Classify whether a project already follows the canonical manuscript lifecycle, has a proof graph ready for manuscript synthesis, is a legacy manuscript, is still research-in-progress, or is internally inconsistent. This is the authoritative preflight for bringing existing graphs into the release system.", "viewer", objectSchema({ workspace_id: s, project_id: s }, ["workspace_id", "project_id"])),
  tool("align_project_release_state", "Idempotently align a proof-complete research graph with the canonical manuscript lifecycle by creating or reusing exactly one PaperWriter synthesis workstream. It never invents approvals, candidates, audit evidence, or publication state. Use only when the release contract permits it.", "editor", objectSchema({ workspace_id: s, project_id: s }, ["workspace_id", "project_id"]), scopes.maffReview),
  ...
  tool("record_review_round", "Atomically record or replay one locked review. First call validate_review_round with the identical payload. Canonical evidence_sections items are {review_type, conclusion, concrete_evidence}; aliases are accepted. The assignment target, exact build, and artifacts are immutable. Identical retry returns the original ReviewRound; divergent retry fails closed.", "editor", objectSchema({ workspace_id: s, workstream_id: s, report_id: s, target_object_type: s, target_object_id: s, reviewer_role: s, verdict: reviewVerdict, review_type: s, target_version: s, scope: anyObj, inspected_artifact_ids: strArray, checked_obligation_ids: strArray, parent_math_reopenable: { type: "boolean" }, prior_approvals_evidence_only: { type: "boolean" }, obligation_checks: { type: "array", items: anyObj }, evidence_sections: { type: "array", items: reviewEvidenceSectionSchema }, issues: strArray, required_changes: strArray, checked_refs: strArray, body_markdown: s, created_by_agent_run_id: s, review_assignment_id: s, submission_token: s }, ["workspace_id", "workstream_id", "verdict", "body_markdown"]), scopes.maffReview),
  tool("validate_review_round", "Read-only dry run for the complete locked ReviewRound payload. It does not write or consume the assignment. Use canonical evidence_sections items {review_type, conclusion, concrete_evidence}; failures return stable code REVIEW_EVIDENCE_INVALID plus exact field_errors.", "editor", objectSchema({ workspace_id: s, workstream_id: s, report_id: s, target_object_type: s, target_object_id: s, reviewer_role: s, verdict: reviewVerdict, review_type: s, target_version: s, scope: anyObj, inspected_artifact_ids: strArray, checked_obligation_ids: strArray, parent_math_reopenable: { type: "boolean" }, prior_approvals_evidence_only: { type: "boolean" }, obligation_checks: { type: "array", items: anyObj }, evidence_sections: { type: "array", items: reviewEvidenceSectionSchema }, issues: strArray, required_changes: strArray, checked_refs: strArray, body_markdown: s, created_by_agent_run_id: s, review_assignment_id: s, submission_token: s }, ["workspace_id", "workstream_id", "verdict", "body_markdown", "created_by_agent_run_id", "review_assignment_id", "submission_token"]), scopes.maffReview),
  ...
  tool("submit_run_outcome", "Idempotently complete an AgentRun with a durable state-owned handoff. Identical or accidental retries return the original outcome. If a supported backend operation prevents completion, set next_action.kind=blocked_by_tooling with reason and unblock_condition; Maff closes the run, cancels its unused lock, and preserves the workstream as explicitly blocked.", "editor", objectSchema({ workspace_id: s, agent_run_id: s, completed_work: strArray, changed_objects: strArray, evidence_generated: strArray, checks_performed: strArray, problems_encountered: strArray, unresolved_uncertainty: strArray, gaps_created: strArray, gaps_resolved: strArray, next_action: anyObj }, ["workspace_id", "agent_run_id", "completed_work", "changed_objects", "evidence_generated", "checks_performed", "problems_encountered", "unresolved_uncertainty", "gaps_created", "gaps_resolved", "next_action"])),
  ...
  tool("get_manuscript_version", "Read an exact ManuscriptVersion with its directly linked immutable physical Artifacts, proof obligations, and explicit working/release/approval semantics. isCanonical is a legacy storage flag; use is_current_working_version, release_assessment_active, and approval_status.", "viewer", objectSchema({ workspace_id: s, manuscript_version_id: s }, ["workspace_id", "manuscript_version_id"])),
  tool("repair_exact_version_citation_metadata", "Provenance-locked metadata-only repair for an exact ManuscriptVersion whose citation fingerprint was incorrectly derived. Parses the exact managed source bundle, verifies the exact PDF and both hashes, uses replay-first idempotency, compare-and-swaps the old fingerprint, preserves all protected manuscript/review/proof/lifecycle state, and returns one immutable repair certificate. It never creates a manuscript version or rebuilds source/PDF.", "editor", objectSchema({ workspace_id: s, project_id: s, manuscript_version_id: s, expected_content_hash: s, expected_old_citation_fingerprint: s, source_artifact_id: s, source_artifact_sha256: s, pdf_artifact_id: s, pdf_artifact_sha256: s, actor_agent_run_id: s, mode: s, citations: { type: "array", items: objectSchema({ key: s, bibitem_latex: s }, ["key", "bibitem_latex"]) }, expected_new_citation_fingerprint: s, idempotency_key: s }, ["workspace_id", "project_id", "manuscript_version_id", "expected_content_hash", "expected_old_citation_fingerprint", "source_artifact_id", "source_artifact_sha256", "pdf_artifact_id", "pdf_artifact_sha256", "actor_agent_run_id"]), scopes.maffReview),
  ...
  tool("build_manuscript", "Deterministically materialize the structured manuscript into internal TeX, bibliography, source bundle, and PDF; attach them automatically and surface nothing. Default activation makes the successful build the current working text only and never confers review approval or submission-candidate status.", "editor", objectSchema({ workspace_id: s, project_id: s, agent_run_id: s, promote: { type: "boolean" } }, ["workspace_id", "project_id", "agent_run_id"])),
  ...
  tool("prepare_external_review_package", "Idempotently package the exact publication candidate PDF and source bundle for third-party review without publishing it or completing the project. Use only when the release contract lists this tool. The returned files are the authoritative review handoff.", "editor", objectSchema({ workspace_id: s, project_id: s, manuscript_version_id: s }, ["workspace_id", "project_id"]), scopes.maffReview),
  tool("publish_manuscript", "Idempotently release the pre-existing exact external-review package after publication readiness and an explicit user decision, mark the project complete, retire residual active work, and attach its final PDF plus exact source bundle directly to the chat. It cannot create or skip the review handoff; repeated calls return the existing identical release.", "editor", objectSchema({ workspace_id: s, project_id: s, manuscript_version_id: s }, ["workspace_id", "project_id"]), scopes.maffReview),
  tool("create_proof_obligation", "Record an atomic mathematical exact-version proof obligation including assumptions, excluded regimes, boundary cases, semantic consequences, dependency/source ledger, and the author's non-verifying assertion. Never use this tool for canonical activation, lifecycle state, approval state, policy interpretation, or workflow governance; those facts come only from get_project_release_contract. Mark load_bearing only during final manuscript assembly; it does not create a separate review task.", "editor", objectSchema({ workspace_id: s, project_id: s, manuscript_version_id: s, title: s, statement_markdown: s, dependencies: { type: "array", items: anyObj }, claim_id: s, source_artifact_id: s, proof_location: s, manuscript_location: s, external_theorems: { type: "array", items: anyObj }, external_assumptions_matched: { type: "boolean" }, exact_manuscript_proof_present: { type: "boolean" }, assumptions: strArray, excluded_regimes: strArray, boundary_cases: strArray, semantic_consequences: strArray, author_assertion: s, required: { type: "boolean" }, load_bearing: { type: "boolean" } }, ["workspace_id", "project_id", "manuscript_version_id", "title", "statement_markdown", "assumptions", "boundary_cases"])),
  tool("adopt_reviewed_manuscript_successor", "The only supported working-successor adoption transition. Atomically makes an exact independently reviewed, cleanly built successor the current canonical working text while preserving its predecessor and leaving verification, proof obligations, lifecycle, freeze, release assessment, and publication state unchanged. Use only with every exact identifier returned by get_project_release_contract; never rebuild, rewrite, duplicate-review, promote to submission candidate, or emulate this operation with lower-level flags.", "editor", objectSchema({ workspace_id: s, project_id: s, expected_current_manuscript_version_id: s, successor_manuscript_version_id: s, supporting_review_round_id: s, paper_build_id: s }, ["workspace_id", "project_id", "expected_current_manuscript_version_id", "successor_manuscript_version_id", "supporting_review_round_id", "paper_build_id"]), scopes.maffReview),
...
export async function callTool(toolName: string, args: any, ctx: ToolContext) {
  ...
  switch (toolName) {
    ...
    case "assess_project_release_alignment": return runtime.assessProjectReleaseAlignment(workspaceId, args.project_id)
    case "align_project_release_state": return runtime.alignProjectReleaseState(workspaceId, args.project_id)
    ...
    case "validate_review_round": return runtime.recordReviewRound({ workspaceId, workstreamId: args.workstream_id, reportId: args.report_id, targetObjectType: args.target_object_type, targetObjectId: args.target_object_id, reviewerRole: args.reviewer_role, verdict: args.verdict, reviewType: args.review_type, targetVersion: args.target_version, scope: args.scope, inspectedArtifactIds: args.inspected_artifact_ids, checkedObligationIds: args.checked_obligation_ids, parentMathReopenable: args.parent_math_reopenable, priorApprovalsEvidenceOnly: args.prior_approvals_evidence_only, obligationChecks: args.obligation_checks, evidenceSections: args.evidence_sections, issues: args.issues, requiredChanges: args.required_changes, checkedRefs: args.checked_refs, bodyMarkdown: args.body_markdown, createdByAgentRunId: args.created_by_agent_run_id, reviewAssignmentId: args.review_assignment_id, submissionToken: args.submission_token, validateOnly: true })
    ...
    case "repair_exact_version_citation_metadata": return runtime.repairExactVersionCitationMetadata({ workspaceId, projectId: args.project_id, manuscriptVersionId: args.manuscript_version_id, expectedContentHash: args.expected_content_hash, expectedOldCitationFingerprint: args.expected_old_citation_fingerprint, sourceArtifactId: args.source_artifact_id, sourceArtifactSha256: args.source_artifact_sha256, pdfArtifactId: args.pdf_artifact_id, pdfArtifactSha256: args.pdf_artifact_sha256, actorAgentRunId: args.actor_agent_run_id, mode: args.mode, citations: args.citations, expectedNewCitationFingerprint: args.expected_new_citation_fingerprint, idempotencyKey: args.idempotency_key })
    ...
    case "prepare_external_review_package": return runtime.prepareExternalReviewPackage({ workspaceId, projectId: args.project_id, manuscriptVersionId: args.manuscript_version_id })
    case "adopt_reviewed_manuscript_successor": return runtime.adoptReviewedManuscriptSuccessor({ workspaceId, projectId: args.project_id, expectedCurrentManuscriptVersionId: args.expected_current_manuscript_version_id, successorManuscriptVersionId: args.successor_manuscript_version_id, supportingReviewRoundId: args.supporting_review_round_id, paperBuildId: args.paper_build_id })
...
export function contentResult(toolName: string, value: unknown) {
  ...
  const embeddedFiles = (structuredContent as any).embedded_files
  if (["prepare_external_review_package", "publish_manuscript"].includes(toolName) && Array.isArray(embeddedFiles) && embeddedFiles.length) {
    const metadata = { ...structuredContent } as any
    delete metadata.embedded_files
    const resources = embeddedFiles.map((file: any) => ({
      type: "resource",
      resource: {
        uri: file.embedded_resource.uri,
        mimeType: file.embedded_resource.mime_type ?? file.mime_type ?? "application/octet-stream",
        blob: file.embedded_resource.blob
      }
    }))
    return { structuredContent: metadata, content: [...resources, { type: "text", text: JSON.stringify(metadata, null, 2) }] }
  }
...
export async function mcpHandler(req: Request, res: Response) {
  try {
    ...
    const err = error as Error & { status?: number; wwwAuthenticate?: string; code?: string; details?: unknown }
    ...
    res.status(err.status ?? 400).json({
      ...
      error: {
        message: err.message,
        data: { ...(err.code ? { code: err.code } : {}), ...(err.details ? { details: err.details } : {}), ...(err.wwwAuthenticate ? { _meta: { "mcp/www_authenticate": err.wwwAuthenticate } } : {}) }
      }
    })
  }
}
