...
import { addInformalProofToClaim, addRouteToClaim, completeWorkflow, computeClaimReadiness, createClaim, createConjecture, createGap, createProblem, createProofRoute, decomposeClaim, logProofAttempt, promoteInlineSubclaimToClaim, promoteRouteToNode, researchExtras, updateClaimLeanStatus, updateClaimRoute } from "./tools/researchTools.js"
...
export const toolDefinitions: ToolDef[] = [
  ...
  tool("promote_route_to_node", "Promote an inline route to a standalone ProofRoute node only when it is substantial or explicitly requested.", "editor", objectSchema({ workspace_id: s, claim_id: s, route_title_or_id: s }, ["workspace_id", "claim_id", "route_title_or_id"])),
  ...
  tool("update_claim_lean_status", "Update the Claim's Lean formalization section and Lean metadata.", "editor", objectSchema({ workspace_id: s, claim_id: s, lean_status: s, lean_file: s, lean_name: s, diagnostics: s, notes: s }, ["workspace_id", "claim_id", "lean_status"])),
  tool("decompose_claim", "Record proposed subclaims/dependencies for a Claim without creating nodes by default.", "editor", objectSchema({ workspace_id: s, claim_id: s, subclaims: strArray }, ["workspace_id", "claim_id", "subclaims"])),
  tool("promote_inline_subclaim_to_claim", "Promote a nontrivial inline subclaim to its own Claim node and link it as a dependency.", "editor", objectSchema({ workspace_id: s, parent_claim_id: s, statement: s, role: s }, ["workspace_id", "parent_claim_id", "statement"])),
...
async function callTool(toolName: string, args: any, ctx: ToolContext) {
  ...
  switch (toolName) {
    ...
    case "create_claim": return createClaim({ workspaceId, problemId: args.problem_id, title: args.title, statement: args.statement, motivation: args.motivation, claimKind: args.claim_kind, role: args.role, confidence: args.confidence, userId })
    ...
    case "promote_route_to_node": return promoteRouteToNode({ workspaceId, claimId: args.claim_id, routeTitleOrId: args.route_title_or_id, userId })
    ...
    case "update_claim_lean_status": return updateClaimLeanStatus({ workspaceId, claimId: args.claim_id, leanStatus: args.lean_status, leanFile: args.lean_file, leanName: args.lean_name, diagnostics: args.diagnostics, notes: args.notes, userId })
    case "decompose_claim": return decomposeClaim({ workspaceId, claimId: args.claim_id, subclaims: args.subclaims, userId })
    case "promote_inline_subclaim_to_claim": return promoteInlineSubclaimToClaim({ workspaceId, parentClaimId: args.parent_claim_id, statement: args.statement, role: args.role, userId })
