...
export function registerNodeRoutes(router: Router) {
  router.get("/workspaces/:id/nodes", asyncHandler(async (req, res) => {
  ...
  router.get("/workspaces/:id/nodes/:nodeId", asyncHandler(async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "viewer")
    res.json(await getNode(req.params.id, req.params.nodeId))
  }))
  router.post("/workspaces/:id/nodes", asyncHandler(async (req, res) => {
    const user = requireUser(req)
  ...
  router.patch("/workspaces/:id/nodes/:nodeId/metadata", asyncHandler(async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
    res.json(await updateNodeMetadataTool({ workspaceId: req.params.id, nodeId: req.params.nodeId, patch: req.body, userId: user.id }))
  }))
  router.post("/workspaces/:id/nodes/:nodeId/append", asyncHandler(async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
  ...
  router.post("/workspaces/:id/nodes/:nodeId/replace-section", asyncHandler(async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
  ...
  router.post("/workspaces/:id/nodes/:nodeId/status", asyncHandler(async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
  ...
  router.get("/workspaces/:id/node-index/:nodeId", asyncHandler(async (req, res) => {
