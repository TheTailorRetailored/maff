...
export function registerLeanRoutes(router: Router) {
  router.post("/workspaces/:id/lean/project", async (req, res) => {
  ...
  router.post("/workspaces/:id/lean/stub", async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
  ...
  router.post("/workspaces/:id/lean/check", async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
    res.json(await leanCheck({ workspaceId: req.params.id, leanFileId: req.body.leanFileId }))
  })
  router.post("/workspaces/:id/lean/goal", async (req, res) => {
    const user = requireUser(req)
    await requireWorkspaceRole(user.id, req.params.id, "editor")
    res.json(await leanGoal({ workspaceId: req.params.id, leanFileId: req.body.leanFileId, position: req.body.position }))
  })
  router.post("/workspaces/:id/lean/search", async (req, res) => res.json(await leanExtras.lean_search_mathlib({ workspaceId: req.params.id, query: req.body.query })))
}
