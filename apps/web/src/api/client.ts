...
export type SubmissionReadiness = { submission_ready: boolean; publication_candidate?: boolean; lifecycle_stage?: string; policy_version?: string; status: string; reasons: string[]; next_required_action?: { gate: string; instruction: string } | null; gates: Record<string, { satisfied: boolean; review_ids?: string[] }>; stale_review_references: unknown[]; missing_gate_references: string[]; blocking_object_references: unknown[] }
...
export type ControlRoom = { project: Project; canonical_working_paper?: ManuscriptVersion | null; readiness?: SubmissionReadiness; project_health?: ProjectHealth; actionability?: { state: string; actionable: boolean }; latest_run_outcome?: { continuationMode: string; continuationReason: string; userPrompt: string; completedWork: unknown[]; problemsEncountered: unknown[] } | null; latest_audit?: { id: string; mode: string; status: string; summaryMarkdown: string; noProjectMutation: boolean; findings: Array<{ id: string; severity: string; title: string; status: string }> } | null; active_repair_campaign?: { id: string; title: string; status: string; tasks: Array<{ id: string; title: string; status: string }> } | null; latest_import?: { id: string; title: string; status: string } | null; workstream_dependency_states?: Array<{ workstream_id: string; satisfied: boolean; blocking_prerequisite_ids: string[] }>; goals_by_status: Record<string, ProjectGoal[]>; workstreams_by_status: Record<string, Workstream[]>; needs_review: Workstream[]; blocked_or_escalated: Workstream[]; recent_agent_runs: AgentRun[]; key_claims: Claim[]; open_gaps: Gap[]; recent_reviews: ReviewRound[]; suggested_next_assignment?: Workstream | null; frontier?: ResearchFrontier }
